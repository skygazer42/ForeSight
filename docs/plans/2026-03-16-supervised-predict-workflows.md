# Supervised Predict Workflows Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add public supervised prediction helpers that expose direct forecast-time feature rows and dense arrays for dataframe-first local lag workflows.

**Architecture:** Keep the public implementation in `src/foresight/data/workflows.py`. Reuse the existing supervised feature ordering and naming conventions so prediction rows align with `make_supervised_frame(..., horizon=h)` feature columns. Export the new helpers from `src/foresight/data/__init__.py` and `src/foresight/__init__.py`, then sync generated API metadata.
...
def test_make_supervised_predict_frame_matches_training_features_at_cutoff() -> None:
    ...
    pred_frame = make_supervised_predict_frame(..., cutoff=...)
...
def test_make_supervised_predict_arrays_matches_predict_frame() -> None:
    ...
    pred_arrays = make_supervised_predict_arrays(...)
...
git commit -m "test: add supervised predict workflow coverage"
```
...
def test_make_supervised_predict_frame_allows_missing_future_y() -> None:
    pred_frame = make_supervised_predict_frame(long_df_with_future_nan_y, ...)
    assert len(pred_frame) == 1


def test_root_import_exports_supervised_predict_workflows() -> None:
    assert hasattr(foresight, "make_supervised_predict_frame")
    assert hasattr(foresight, "make_supervised_predict_arrays")
```
...
git commit -m "test: cover supervised predict exports"
```
...
- preserve the same feature naming and ordering as `make_supervised_frame(..., horizon=h)`
- emit one prediction row per eligible series and cutoff
- support missing future `y` after the cutoff
- require finite history `y` and finite first-target `x_cols`
...
git commit -m "feat: add supervised predict workflows"
```
...
git commit -m "docs: sync supervised predict workflow metadata"
```
