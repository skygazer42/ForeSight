...
def _forecast_from_fixture(x: np.ndarray, horizon: int, payload: dict[str, Any]) -> np.ndarray:
    ...
    damp = float(payload.get("trend_damp", 1.0))
    ...
    for idx in range(h):
        step = float(idx + 1)
        out[idx] = base + ((trend * damp) * step if use_trend else 0.0)
    return out


def foundation_wrapper_b_forecast(
    train: Any,
    horizon: int,
    *,
    family: str,
    ...
    **params: Any,
    ...
    family_s = str(family).strip() or "foundation-wrapper-b"
