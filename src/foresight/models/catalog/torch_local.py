...
def _make_wave106_transformer_refinement_local_strategy_presets(
    context: Any, catalog: dict[str, Any]
) -> dict[str, ModelSpec]:
    model_spec = context.ModelSpec
    extra: dict[str, ModelSpec] = {}
    ...
    preset_specs = {
        "torch-informer-sam-direct": (
            "torch-informer-direct",
            "Torch Informer direct forecaster with SAM plus cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-autoformer-regularized-direct": (
            "torch-autoformer-direct",
            "Torch Autoformer direct forecaster with dropout-heavy regularized training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 3,
        ...
        "torch-fedformer-lookahead-direct": (
            "torch-fedformer-direct",
            "Torch FEDformer direct forecaster with Lookahead-optimized cosine training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-crossformer-ema-direct": (
            "torch-crossformer-direct",
            "Torch Crossformer direct forecaster with an EMA-stabilized cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-itransformer-swa-direct": (
            "torch-itransformer-direct",
            "Torch iTransformer direct forecaster with cosine-restarts plus SWA training recipe for flatter minima. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine_restarts",
                "scheduler_restart_period": 8,
        ...
        "torch-timexer-ema-direct": (
            "torch-timexer-direct",
            "Torch TimeXer direct forecaster with an EMA-stabilized cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
...
def _make_wave107_state_space_refinement_local_strategy_presets(
    context: Any, catalog: dict[str, Any]
) -> dict[str, ModelSpec]:
    model_spec = context.ModelSpec
    extra: dict[str, ModelSpec] = {}
    ...
    preset_specs = {
        "torch-s4-ema-direct": (
            "torch-s4-direct",
            "Torch S4 direct forecaster with an EMA-stabilized cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-s4d-swa-direct": (
            "torch-s4d-direct",
            "Torch S4D direct forecaster with cosine-restarts plus SWA training recipe for flatter minima. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine_restarts",
                "scheduler_restart_period": 8,
        ...
        "torch-s5-sam-direct": (
            "torch-s5-direct",
            "Torch S5 direct forecaster with SAM plus cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-mamba2-regularized-direct": (
            "torch-mamba2-direct",
            "Torch Mamba-2 direct forecaster with dropout-heavy regularized training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 3,
        ...
        "torch-timesmamba-lookahead-direct": (
            "torch-timesmamba-direct",
            "Torch TimesMamba direct forecaster with Lookahead-optimized cosine training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-pathformer-swa-direct": (
            "torch-pathformer-direct",
            "Torch Pathformer direct forecaster with cosine-restarts plus SWA training recipe for flatter minima. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine_restarts",
                "scheduler_restart_period": 8,
...
def _make_wave110_sequence_refinement_local_strategy_presets(
    context: Any, catalog: dict[str, Any]
) -> dict[str, ModelSpec]:
    model_spec = context.ModelSpec
    extra: dict[str, ModelSpec] = {}
    ...
    preset_specs = {
        "torch-patchtst-lookahead-direct": (
            "torch-patchtst-direct",
            "Torch PatchTST direct forecaster with Lookahead-optimized cosine training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-retnet-swa-direct": (
            "torch-retnet-direct",
            "Torch RetNet direct forecaster with cosine-restarts plus SWA training recipe for flatter minima. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine_restarts",
                "scheduler_restart_period": 8,
        ...
        "torch-timesnet-lookahead-direct": (
            "torch-timesnet-direct",
            "Torch TimesNet direct forecaster with Lookahead-optimized cosine training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-seq2seq-gru-regularized-direct": (
            "torch-seq2seq-gru-direct",
            "Torch Seq2Seq GRU direct forecaster with dropout-heavy regularized training defaults. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 3,
        ...
        "torch-seq2seq-gru-sam-direct": (
            "torch-seq2seq-gru-direct",
            "Torch Seq2Seq GRU direct forecaster with SAM plus cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
        ...
        "torch-seq2seq-attn-lstm-ema-direct": (
            "torch-seq2seq-attn-lstm-direct",
            "Torch Seq2Seq attention-LSTM direct forecaster with an EMA-stabilized cosine-warmup training recipe. Requires PyTorch.",
            {
                "optimizer": "adamw",
                "scheduler": "cosine",
                "warmup_epochs": 2,
...
def build_torch_local_catalog(context: Any) -> dict[str, Any]:
    ...
    catalog.update(_make_wave110_sequence_refinement_local_strategy_presets(context, catalog))
    return catalog
