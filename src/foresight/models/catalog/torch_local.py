...
def build_torch_local_catalog(context: Any) -> dict[str, Any]:
    ...
    _factory_torch_nonstationary_transformer_direct = context._factory_torch_nonstationary_transformer_direct
    ...
    "torch-perceiver-direct": ModelSpec(
        ...
        param_help={
            "lags": "Lag window length",
            "d_model": "Latent and token embedding dimension",
            "latent_len": "Number of learned latent tokens",
            "nhead": "Number of cross-attention / latent attention heads",
            "num_layers": "Number of latent Transformer encoder layers",
            "dim_feedforward": "Latent Transformer feed-forward dimension",
            "dropout": "Dropout probability in [0,1)",
            **_TORCH_COMMON_PARAM_HELP,
        },
        requires=("torch",),
    ...
    "torch-segrnn-direct": ModelSpec(
        ...
        param_help={
            "lags": "Lag window length",
            "segment_len": "Segment length used to chunk the lag window",
            "d_model": "Segment embedding dimension",
            "hidden_size": "GRU hidden size over segment tokens",
            "num_layers": "Number of stacked GRU layers",
            "dropout": "Dropout probability in [0,1) (only if num_layers>1 inside GRU)",
            **_TORCH_COMMON_PARAM_HELP,
        },
        requires=("torch",),
    ),
    ...
    "torch-moderntcn-direct": ModelSpec(
        ...
        param_help={
            "lags": "Lag window length",
            "patch_len": "Number of timesteps per patch token",
            "d_model": "Patch embedding dimension",
            "num_blocks": "Number of ModernTCN mixer blocks",
            "expansion_factor": "Expansion ratio used by the channel MLP",
            "kernel_size": "Odd depthwise convolution kernel size over patch tokens",
            "dropout": "Dropout probability in [0,1)",
            **_TORCH_COMMON_PARAM_HELP,
        },
        requires=("torch",),
    ...
    "torch-basisformer-direct": ModelSpec(
        ...
        param_help={
            "lags": "Lag window length",
            "patch_len": "Number of timesteps per patch token",
            "d_model": "Patch embedding / latent dimension",
