...
def build_stats_catalog(context: Any) -> dict[str, Any]:
    ...
    CANDIDATE_STATIONARITY_PARAM_HELP = "Enforce stationarity for candidate models (true/false)"
    CANDIDATE_INVERTIBILITY_PARAM_HELP = "Enforce invertibility for candidate models (true/false)"
    MODEL_SELECTION_CRITERION_PARAM_HELP = "Model selection criterion: aic or bic"
    FOURIER_PERIODS_PARAM_HELP = "Comma-separated seasonal periods for Fourier terms (e.g. 7,365)"
    FOURIER_ORDERS_PARAM_HELP = "Fourier order per period (int or comma-separated list)"
    LOCAL_LEVEL = "local level"
    STL_PERIOD_PARAM_HELP = "Seasonal period for STL"
    STL_SEASONAL_PARAM_HELP = "STL seasonal smoother length (odd integer >= 3; default 7)"
    ...
    OPTIONAL_BOXCOX_LAMBDA_PARAM_HELP = "Optional Box-Cox lambda (float); requires y > 0"
    return {
        "ets": model_spec(
            key="ets",
