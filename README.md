...
- `torch-xformer-*-direct`（requires `.[torch]`）：可配置 Transformer-family（full/local/performer/linformer/nystrom/probsparse/autocorr + RoPE/sincos/Time2Vec + RMSNorm/SwiGLU/RevIN…）
...
- `torch-xformer-*-global`（requires `.[torch]`）：全局/面板 Transformer-family（支持 covariates + time features；attn 支持 probsparse/autocorr 等）
