...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-timesnet-ema-global", {"warmup_epochs": 1}),
        ("torch-timesnet-swa-global", {"swa_start_epoch": 0}),
        ("torch-timesnet-sam-global", {"warmup_epochs": 1}),
        ("torch-timesnet-regularized-global", {"warmup_epochs": 1}),
        ("torch-timesnet-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-timesnet-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave88_global_timesnet_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
