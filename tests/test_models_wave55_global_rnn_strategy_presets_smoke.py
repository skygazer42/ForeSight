...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-rnn-lstm-ema-global", {"warmup_epochs": 1}),
        ("torch-rnn-gru-swa-global", {"swa_start_epoch": 0}),
        ("torch-rnn-lstm-sam-global", {"warmup_epochs": 1}),
        ("torch-rnn-gru-regularized-global", {"warmup_epochs": 1}),
        ("torch-rnn-encoder-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-rnn-encoder-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave55_global_rnn_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
