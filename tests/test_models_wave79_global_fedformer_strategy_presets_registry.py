...
def test_wave79_global_fedformer_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_FEDFORMER_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave79_global_fedformer_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_FEDFORMER_STRATEGY_PRESET_KEYS:
...
def test_wave79_global_fedformer_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-fedformer-ema-global")
    swa = get_model_spec("torch-fedformer-swa-global")
    sam = get_model_spec("torch-fedformer-sam-global")
    regularized = get_model_spec("torch-fedformer-regularized-global")
    longhorizon = get_model_spec("torch-fedformer-longhorizon-global")
    lookahead = get_model_spec("torch-fedformer-lookahead-global")
