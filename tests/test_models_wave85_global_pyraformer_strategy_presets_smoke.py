...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-pyraformer-ema-global", {"warmup_epochs": 1}),
        ("torch-pyraformer-swa-global", {"swa_start_epoch": 0}),
        ("torch-pyraformer-sam-global", {"warmup_epochs": 1}),
        ("torch-pyraformer-regularized-global", {"warmup_epochs": 1}),
        ("torch-pyraformer-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-pyraformer-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave85_global_pyraformer_strategy_presets_smoke(
