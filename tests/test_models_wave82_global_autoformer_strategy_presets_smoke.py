...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-autoformer-ema-global", {"warmup_epochs": 1}),
        ("torch-autoformer-swa-global", {"swa_start_epoch": 0}),
        ("torch-autoformer-sam-global", {"warmup_epochs": 1}),
        ("torch-autoformer-regularized-global", {"warmup_epochs": 1}),
        ("torch-autoformer-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-autoformer-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave82_global_autoformer_strategy_presets_smoke(
