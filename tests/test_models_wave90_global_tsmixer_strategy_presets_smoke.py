...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-tsmixer-ema-global", {"warmup_epochs": 1}),
        ("torch-tsmixer-swa-global", {"swa_start_epoch": 0}),
        ("torch-tsmixer-sam-global", {"warmup_epochs": 1}),
        ("torch-tsmixer-regularized-global", {"warmup_epochs": 1}),
        ("torch-tsmixer-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-tsmixer-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave90_global_tsmixer_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
