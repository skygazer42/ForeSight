...
def test_wave88_global_timesnet_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_TIMESNET_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave88_global_timesnet_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_TIMESNET_STRATEGY_PRESET_KEYS:
...
def test_wave88_global_timesnet_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-timesnet-ema-global")
    swa = get_model_spec("torch-timesnet-swa-global")
    sam = get_model_spec("torch-timesnet-sam-global")
    regularized = get_model_spec("torch-timesnet-regularized-global")
    longhorizon = get_model_spec("torch-timesnet-longhorizon-global")
    lookahead = get_model_spec("torch-timesnet-lookahead-global")
