...
def test_wave59_global_dlinear_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_DLINEAR_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave59_global_dlinear_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_DLINEAR_STRATEGY_PRESET_KEYS:
...
def test_wave59_global_dlinear_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-dlinear-ema-global")
    swa = get_model_spec("torch-dlinear-swa-global")
    sam = get_model_spec("torch-dlinear-sam-global")
    regularized = get_model_spec("torch-dlinear-regularized-global")
    longhorizon = get_model_spec("torch-dlinear-longhorizon-global")
    lookahead = get_model_spec("torch-dlinear-lookahead-global")
