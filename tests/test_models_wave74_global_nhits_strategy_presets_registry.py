...
def test_wave74_global_nhits_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_NHITS_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave74_global_nhits_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_NHITS_STRATEGY_PRESET_KEYS:
...
def test_wave74_global_nhits_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-nhits-ema-global")
    swa = get_model_spec("torch-nhits-swa-global")
    sam = get_model_spec("torch-nhits-sam-global")
    regularized = get_model_spec("torch-nhits-regularized-global")
    longhorizon = get_model_spec("torch-nhits-longhorizon-global")
    lookahead = get_model_spec("torch-nhits-lookahead-global")
