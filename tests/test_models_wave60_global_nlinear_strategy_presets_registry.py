...
def test_wave60_global_nlinear_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_NLINEAR_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave60_global_nlinear_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_NLINEAR_STRATEGY_PRESET_KEYS:
...
def test_wave60_global_nlinear_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-nlinear-ema-global")
    swa = get_model_spec("torch-nlinear-swa-global")
    sam = get_model_spec("torch-nlinear-sam-global")
    regularized = get_model_spec("torch-nlinear-regularized-global")
    longhorizon = get_model_spec("torch-nlinear-longhorizon-global")
    lookahead = get_model_spec("torch-nlinear-lookahead-global")
