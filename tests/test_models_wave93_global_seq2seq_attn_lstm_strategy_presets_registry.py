...
def test_wave93_global_seq2seq_attn_lstm_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_SEQ2SEQ_ATTN_LSTM_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave93_global_seq2seq_attn_lstm_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_SEQ2SEQ_ATTN_LSTM_STRATEGY_PRESET_KEYS:
...
def test_wave93_global_seq2seq_attn_lstm_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-seq2seq-attn-lstm-ema-global")
    swa = get_model_spec("torch-seq2seq-attn-lstm-swa-global")
    sam = get_model_spec("torch-seq2seq-attn-lstm-sam-global")
    regularized = get_model_spec("torch-seq2seq-attn-lstm-regularized-global")
    longhorizon = get_model_spec("torch-seq2seq-attn-lstm-longhorizon-global")
    lookahead = get_model_spec("torch-seq2seq-attn-lstm-lookahead-global")
