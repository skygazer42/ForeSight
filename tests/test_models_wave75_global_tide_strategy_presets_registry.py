...
def test_wave75_global_tide_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_TIDE_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave75_global_tide_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_TIDE_STRATEGY_PRESET_KEYS:
...
def test_wave75_global_tide_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-tide-ema-global")
    swa = get_model_spec("torch-tide-swa-global")
    sam = get_model_spec("torch-tide-sam-global")
    regularized = get_model_spec("torch-tide-regularized-global")
    longhorizon = get_model_spec("torch-tide-longhorizon-global")
    lookahead = get_model_spec("torch-tide-lookahead-global")
