...
def test_wave76_global_wavenet_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_WAVENET_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave76_global_wavenet_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_WAVENET_STRATEGY_PRESET_KEYS:
...
def test_wave76_global_wavenet_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-wavenet-ema-global")
    swa = get_model_spec("torch-wavenet-swa-global")
    sam = get_model_spec("torch-wavenet-sam-global")
    regularized = get_model_spec("torch-wavenet-regularized-global")
    longhorizon = get_model_spec("torch-wavenet-longhorizon-global")
    lookahead = get_model_spec("torch-wavenet-lookahead-global")
