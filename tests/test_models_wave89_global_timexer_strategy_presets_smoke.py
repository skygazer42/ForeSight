...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-timexer-ema-global", {"warmup_epochs": 1}),
        ("torch-timexer-swa-global", {"swa_start_epoch": 0}),
        ("torch-timexer-sam-global", {"warmup_epochs": 1}),
        ("torch-timexer-regularized-global", {"warmup_epochs": 1}),
        ("torch-timexer-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-timexer-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave89_global_timexer_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
