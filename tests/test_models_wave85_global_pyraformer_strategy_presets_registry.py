...
def test_wave85_global_pyraformer_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_PYRAFORMER_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave85_global_pyraformer_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_PYRAFORMER_STRATEGY_PRESET_KEYS:
...
def test_wave85_global_pyraformer_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-pyraformer-ema-global")
    swa = get_model_spec("torch-pyraformer-swa-global")
    sam = get_model_spec("torch-pyraformer-sam-global")
    regularized = get_model_spec("torch-pyraformer-regularized-global")
    longhorizon = get_model_spec("torch-pyraformer-longhorizon-global")
    lookahead = get_model_spec("torch-pyraformer-lookahead-global")
