...
def test_wave90_global_tsmixer_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_TSMIXER_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave90_global_tsmixer_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_TSMIXER_STRATEGY_PRESET_KEYS:
...
def test_wave90_global_tsmixer_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-tsmixer-ema-global")
    swa = get_model_spec("torch-tsmixer-swa-global")
    sam = get_model_spec("torch-tsmixer-sam-global")
    regularized = get_model_spec("torch-tsmixer-regularized-global")
    longhorizon = get_model_spec("torch-tsmixer-longhorizon-global")
    lookahead = get_model_spec("torch-tsmixer-lookahead-global")
