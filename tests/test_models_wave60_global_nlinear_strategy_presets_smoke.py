...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-nlinear-ema-global", {"warmup_epochs": 1}),
        ("torch-nlinear-swa-global", {"swa_start_epoch": 0}),
        ("torch-nlinear-sam-global", {"warmup_epochs": 1}),
        ("torch-nlinear-regularized-global", {"warmup_epochs": 1}),
        ("torch-nlinear-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-nlinear-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave60_global_nlinear_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
