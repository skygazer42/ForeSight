...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-kan-ema-global", {"warmup_epochs": 1}),
        ("torch-kan-swa-global", {"swa_start_epoch": 0}),
        ("torch-kan-sam-global", {"warmup_epochs": 1}),
        ("torch-kan-regularized-global", {"warmup_epochs": 1}),
        ("torch-kan-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-kan-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave68_global_kan_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
