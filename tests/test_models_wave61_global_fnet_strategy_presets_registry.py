...
def test_wave61_global_fnet_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_FNET_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave61_global_fnet_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_FNET_STRATEGY_PRESET_KEYS:
...
def test_wave61_global_fnet_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-fnet-ema-global")
    swa = get_model_spec("torch-fnet-swa-global")
    sam = get_model_spec("torch-fnet-sam-global")
    regularized = get_model_spec("torch-fnet-regularized-global")
    longhorizon = get_model_spec("torch-fnet-longhorizon-global")
    lookahead = get_model_spec("torch-fnet-lookahead-global")
