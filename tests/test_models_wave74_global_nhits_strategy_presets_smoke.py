...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-nhits-ema-global", {"warmup_epochs": 1}),
        ("torch-nhits-swa-global", {"swa_start_epoch": 0}),
        ("torch-nhits-sam-global", {"warmup_epochs": 1}),
        ("torch-nhits-regularized-global", {"warmup_epochs": 1}),
        ("torch-nhits-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-nhits-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave74_global_nhits_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
