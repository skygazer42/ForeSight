...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-dlinear-ema-global", {"warmup_epochs": 1}),
        ("torch-dlinear-swa-global", {"swa_start_epoch": 0}),
        ("torch-dlinear-sam-global", {"warmup_epochs": 1}),
        ("torch-dlinear-regularized-global", {"warmup_epochs": 1}),
        ("torch-dlinear-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-dlinear-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave59_global_dlinear_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
