...
def test_wave71_global_esrnn_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_ESRNN_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave71_global_esrnn_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_ESRNN_STRATEGY_PRESET_KEYS:
...
def test_wave71_global_esrnn_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-esrnn-ema-global")
    swa = get_model_spec("torch-esrnn-swa-global")
    sam = get_model_spec("torch-esrnn-sam-global")
    regularized = get_model_spec("torch-esrnn-regularized-global")
    longhorizon = get_model_spec("torch-esrnn-longhorizon-global")
    lookahead = get_model_spec("torch-esrnn-lookahead-global")
