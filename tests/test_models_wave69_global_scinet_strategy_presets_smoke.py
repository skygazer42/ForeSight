...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-scinet-ema-global", {"warmup_epochs": 1}),
        ("torch-scinet-swa-global", {"swa_start_epoch": 0}),
        ("torch-scinet-sam-global", {"warmup_epochs": 1}),
        ("torch-scinet-regularized-global", {"warmup_epochs": 1}),
        ("torch-scinet-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-scinet-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave69_global_scinet_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
