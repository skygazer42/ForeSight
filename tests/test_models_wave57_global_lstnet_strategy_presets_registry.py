...
def test_wave57_global_lstnet_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_LSTNET_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave57_global_lstnet_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_LSTNET_STRATEGY_PRESET_KEYS:
...
def test_wave57_global_lstnet_strategy_preset_defaults() -> None:
    ...
    swa = get_model_spec("torch-lstnet-swa-global")
    sam = get_model_spec("torch-lstnet-sam-global")
    regularized = get_model_spec("torch-lstnet-regularized-global")
    longhorizon = get_model_spec("torch-lstnet-longhorizon-global")
    lookahead = get_model_spec("torch-lstnet-lookahead-global")
