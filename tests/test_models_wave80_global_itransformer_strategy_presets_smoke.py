...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-itransformer-ema-global", {"warmup_epochs": 1}),
        ("torch-itransformer-swa-global", {"swa_start_epoch": 0}),
        ("torch-itransformer-sam-global", {"warmup_epochs": 1}),
        ("torch-itransformer-regularized-global", {"warmup_epochs": 1}),
        ("torch-itransformer-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-itransformer-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave80_global_itransformer_strategy_presets_smoke(
