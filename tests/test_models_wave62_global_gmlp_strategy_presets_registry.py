...
def test_wave62_global_gmlp_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_GMLP_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave62_global_gmlp_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_GMLP_STRATEGY_PRESET_KEYS:
...
def test_wave62_global_gmlp_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-gmlp-ema-global")
    swa = get_model_spec("torch-gmlp-swa-global")
    sam = get_model_spec("torch-gmlp-sam-global")
    regularized = get_model_spec("torch-gmlp-regularized-global")
    longhorizon = get_model_spec("torch-gmlp-longhorizon-global")
    lookahead = get_model_spec("torch-gmlp-lookahead-global")
