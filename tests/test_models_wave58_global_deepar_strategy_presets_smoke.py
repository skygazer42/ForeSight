...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-deepar-ema-global", {"warmup_epochs": 1}),
        ("torch-deepar-swa-global", {"swa_start_epoch": 0}),
        ("torch-deepar-sam-global", {"warmup_epochs": 1}),
        ("torch-deepar-regularized-global", {"warmup_epochs": 1}),
        ("torch-deepar-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-deepar-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave58_global_deepar_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
