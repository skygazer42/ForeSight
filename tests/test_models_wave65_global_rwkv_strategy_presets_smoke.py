...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-rwkv-ema-global", {"warmup_epochs": 1}),
        ("torch-rwkv-swa-global", {"swa_start_epoch": 0}),
        ("torch-rwkv-sam-global", {"warmup_epochs": 1}),
        ("torch-rwkv-regularized-global", {"warmup_epochs": 1}),
        ("torch-rwkv-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-rwkv-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave65_global_rwkv_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
