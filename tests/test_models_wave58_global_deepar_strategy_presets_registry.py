...
def test_wave58_global_deepar_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_DEEPAR_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave58_global_deepar_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_DEEPAR_STRATEGY_PRESET_KEYS:
...
def test_wave58_global_deepar_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-deepar-ema-global")
    swa = get_model_spec("torch-deepar-swa-global")
    sam = get_model_spec("torch-deepar-sam-global")
    regularized = get_model_spec("torch-deepar-regularized-global")
    longhorizon = get_model_spec("torch-deepar-longhorizon-global")
    lookahead = get_model_spec("torch-deepar-lookahead-global")
    ...
    assert longhorizon.default_params["ema_decay"] == 0.99
