...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-hyena-ema-global", {"warmup_epochs": 1}),
        ("torch-hyena-swa-global", {"swa_start_epoch": 0}),
        ("torch-hyena-sam-global", {"warmup_epochs": 1}),
        ("torch-hyena-regularized-global", {"warmup_epochs": 1}),
        ("torch-hyena-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-hyena-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave66_global_hyena_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
