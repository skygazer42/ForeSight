...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-esrnn-ema-global", {"warmup_epochs": 1}),
        ("torch-esrnn-swa-global", {"swa_start_epoch": 0}),
        ("torch-esrnn-sam-global", {"warmup_epochs": 1}),
        ("torch-esrnn-regularized-global", {"warmup_epochs": 1}),
        ("torch-esrnn-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-esrnn-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave71_global_esrnn_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
