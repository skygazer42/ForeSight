...
def test_wave65_global_rwkv_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_RWKV_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave65_global_rwkv_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_RWKV_STRATEGY_PRESET_KEYS:
...
def test_wave65_global_rwkv_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-rwkv-ema-global")
    swa = get_model_spec("torch-rwkv-swa-global")
    sam = get_model_spec("torch-rwkv-sam-global")
    regularized = get_model_spec("torch-rwkv-regularized-global")
    longhorizon = get_model_spec("torch-rwkv-longhorizon-global")
    lookahead = get_model_spec("torch-rwkv-lookahead-global")
