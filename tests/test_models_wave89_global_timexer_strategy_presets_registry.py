...
def test_wave89_global_timexer_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_TIMEXER_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave89_global_timexer_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_TIMEXER_STRATEGY_PRESET_KEYS:
...
def test_wave89_global_timexer_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-timexer-ema-global")
    swa = get_model_spec("torch-timexer-swa-global")
    sam = get_model_spec("torch-timexer-sam-global")
    regularized = get_model_spec("torch-timexer-regularized-global")
    longhorizon = get_model_spec("torch-timexer-longhorizon-global")
    lookahead = get_model_spec("torch-timexer-lookahead-global")
