...
def test_wave63_global_ssm_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_SSM_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave63_global_ssm_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_SSM_STRATEGY_PRESET_KEYS:
...
def test_wave63_global_ssm_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-ssm-ema-global")
    swa = get_model_spec("torch-ssm-swa-global")
    sam = get_model_spec("torch-ssm-sam-global")
    regularized = get_model_spec("torch-ssm-regularized-global")
    longhorizon = get_model_spec("torch-ssm-longhorizon-global")
    lookahead = get_model_spec("torch-ssm-lookahead-global")
