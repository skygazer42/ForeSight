...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-informer-ema-global", {"warmup_epochs": 1}),
        ("torch-informer-swa-global", {"swa_start_epoch": 0}),
        ("torch-informer-sam-global", {"warmup_epochs": 1}),
        ("torch-informer-regularized-global", {"warmup_epochs": 1}),
        ("torch-informer-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-informer-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave81_global_informer_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
