...
@pytest.mark.parametrize(
    ("key", "overrides"),
    (
        ("torch-patchtst-ema-global", {"warmup_epochs": 1}),
        ("torch-patchtst-swa-global", {"swa_start_epoch": 0}),
        ("torch-patchtst-sam-global", {"warmup_epochs": 1}),
        ("torch-patchtst-regularized-global", {"warmup_epochs": 1}),
        ("torch-patchtst-longhorizon-global", {"warmup_epochs": 1}),
        ("torch-patchtst-lookahead-global", {"warmup_epochs": 1}),
    ),
)
def test_wave87_global_patchtst_strategy_presets_smoke(key: str, overrides: dict[str, int]) -> None:
