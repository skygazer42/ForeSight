...
def test_wave67_global_dilated_rnn_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_DILATED_RNN_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave67_global_dilated_rnn_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_DILATED_RNN_STRATEGY_PRESET_KEYS:
...
def test_wave67_global_dilated_rnn_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-dilated-rnn-ema-global")
    swa = get_model_spec("torch-dilated-rnn-swa-global")
    sam = get_model_spec("torch-dilated-rnn-sam-global")
    regularized = get_model_spec("torch-dilated-rnn-regularized-global")
    longhorizon = get_model_spec("torch-dilated-rnn-longhorizon-global")
    lookahead = get_model_spec("torch-dilated-rnn-lookahead-global")
