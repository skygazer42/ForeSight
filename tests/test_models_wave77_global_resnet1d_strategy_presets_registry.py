...
def test_wave77_global_resnet1d_strategy_presets_are_registered() -> None:
    keys = set(list_models())
    for key in GLOBAL_RESNET1D_STRATEGY_PRESET_KEYS:
        assert key in keys


def test_wave77_global_resnet1d_strategy_presets_are_global_torch_optional() -> None:
    for key in GLOBAL_RESNET1D_STRATEGY_PRESET_KEYS:
...
def test_wave77_global_resnet1d_strategy_preset_defaults() -> None:
    ema = get_model_spec("torch-resnet1d-ema-global")
    swa = get_model_spec("torch-resnet1d-swa-global")
    sam = get_model_spec("torch-resnet1d-sam-global")
    regularized = get_model_spec("torch-resnet1d-regularized-global")
    longhorizon = get_model_spec("torch-resnet1d-longhorizon-global")
    lookahead = get_model_spec("torch-resnet1d-lookahead-global")
